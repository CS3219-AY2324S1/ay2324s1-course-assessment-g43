...
const handleCancelRequest = async (
  ...
  if (pendingSocketRequests[index] === socketId) {
    ...
    channel.sendToQueue(
      MATCH_REPLY_QUEUE,
      Buffer.from(JSON.stringify(matchCancelMesssage))
    );
  }
};

const handleSaveMatchRequest = async (
  channel,
  index,
  socketId,
  userId,
...
const handleSuccessMatchRequest = async (
  channel,
  index,
  socketId,
  userId,
  ...
  channel.sendToQueue(
    MATCH_REPLY_QUEUE,
    Buffer.from(JSON.stringify(matchSuccessMessage))
  );
};
  ...
  channel.consume(
    ...
    async (message) => {
      ...
      firstUserSocket.emit(
        ...
        async (session) => {
          if (!session) {
            io.to(firstUserSocketId).emit(
              "match-failure",
              "Failed to create session! User is possibly in another room"
            );
            io.to(secondUserSocketId).emit(
              "match-failure",
              "Failed to create session! User is possibly in another room"
            );
            return;
